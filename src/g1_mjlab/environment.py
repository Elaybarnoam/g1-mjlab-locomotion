...
def build_train_config(
    ...
    *,
    ...
    agent.num_steps_per_env = config.rollout_steps
    agent.max_iterations = config.max_iterations
    agent.save_interval = config.save_interval
    agent.clip_actions = config.action_clip
    if ppo_profile is not None:
        distribution_cfg = agent.actor.distribution_cfg
        ...
        if ppo_profile.entropy_coef is not None:
            agent.algorithm.entropy_coef = ppo_profile.entropy_coef
    agent.experiment_name = task.experiment_name
    agent.run_name = config.run_name
