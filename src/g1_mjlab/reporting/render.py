...
def render_report(run_dir: Path, output_dir: Path | None = None, *, final: bool = False) -> Path:
    ...
    is_walking = config.get("task_id") == "G1-Walking-Flat-v1"
    passed = int(evaluation.get("passed_both" if is_walking else "passed", 0))
    ...
    if is_walking:
        trial_heading = "<th>Trial</th><th>Function / style</th><th>Survival (s)</th><th>Steps</th><th>Command RMS (m/s)</th><th>Classification</th>"
        trial_rows = (
            "".join(
                "<tr><td>{}</td><td>{}/{}</td><td>{:.2f}</td><td>{}</td><td>{}</td><td>{}</td></tr>".format(
                    item.get("trial_id", ""),
                    "pass" if item.get("functional_passed") else "fail",
                    "pass" if item.get("style_passed") else "fail",
                    float(item.get("survived_seconds", 0)),
                    item.get("completed_steps", 0),
                    _safe(item.get("command_rms_m_s", "—")),
                    _safe(item.get("classification", "—")),
                )
                for item in evaluation.get("trials", [])
            )
            or "<tr><td colspan='6'>No evaluation trials recorded</td></tr>"
        )
        behavior_heading = "Walking function and style"
        behavior_summary = (
            f"Combined functional/style passes: {passed}/{planned}. "
            "Missing steps or gait samples fail rather than receiving a favorable zero score."
        )
    else:
        trial_heading = "<th>Trial</th><th>Outcome</th><th>Survival (s)</th><th>Max drift (m)</th><th>Reason</th>"
        trial_rows = (
            "".join(
        ...
        behavior_heading = "Standing versus survival"
        behavior_summary = f"Strict standing passes: {passed}/{planned}. Survival-only passes: {_safe(evaluation.get('survival_passed', 'not separately recorded'))}. Standing success Wilson 95% interval: {_safe(evaluation.get('standing_success_wilson_95', 'unavailable'))}."
    termination_event = -10 if is_walking else -5
    contract_rows = (
        "".join(
...
<h2>What happened</h2><p>{outcome} Behavioral evaluation is separate: {passed} of {planned} recorded trials passed. Training completion is not a behavior-success claim.</p><p>Evaluation phase: {evaluation_phase}. Criteria and provenance: <code>{_safe(evaluation.get("criteria", "Legacy survival-only evaluation"))}</code>.</p>
...
<h2>{behavior_heading}</h2><p>{behavior_summary} Phase: {evaluation_phase}. Development selection and untouched final testing are stored separately.</p>
